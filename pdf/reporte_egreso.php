...
$html .= '</table>';
    // FINAL TABLA DE LAS DETENCIONES



// # INICIO DE TABLA DE EGRESOS ASOCIADOS
    $html .= '<table>';

    // Fila con el título
    $html .= '<tr>';
    $html .= '<td colspan="7" style="text-align: center; font-weight: bold; background-color:#b5b5b5">HISTORIAL DE EGRESOS</td>';
    $html .= '</tr>';
    ...
    $anchoColumnas = array(3,10, 10, 15, 15, 15, 32);
    for ($i = 0; $i < count($columnas); $i++) {
        $html .= '<td style="width: ' . $anchoColumnas[$i] . '%;">' . $columnas[$i] . '</td>';
    }
    $html .= '</tr>';
    ...
    // TRAER LOS EGRESOS ASOCIADAS

    $sql = "SELECT detenciones.*, detalle_detenciones.*, direcciones.*, detenidos.detenido_id, organismos.nombre AS organismo, delitos.nombre AS delito, delitos.descripcion AS delito_desc, egreso_detenciones.*, egreso_detenciones.descripcion AS descripcion_egreso, tipo_egresos.nombre AS tipo_egreso
                FROM detenciones 
                LEFT JOIN detalle_detenciones ON detalle_detenciones.detencion_idfk = detenciones.detencion_id 
                LEFT JOIN detenidos ON detenciones.detenido_idfk = detenidos.detenido_id
                LEFT JOIN direcciones ON detalle_detenciones.direccion_idfk = direcciones.direccion_id
                LEFT JOIN organismos ON detenciones.organismo_idfk = organismos.organismo_id
                LEFT JOIN delitos ON detenciones.delito_idfk = delitos.delito_id
                LEFT JOIN egreso_detenciones ON detenciones.detencion_id = egreso_detenciones.detencion_idfk
                LEFT JOIN tipo_egresos ON egreso_detenciones.tipoegreso_idfk = tipo_egresos.tipoegreso_id
                WHERE detenidos.detenido_id = '$id'
                AND detenciones.estatus = 'EGRESADO'
                AND detenciones.oculto = '0'
                ORDER BY detenciones.detencion_id DESC";
    $resultado_egreso = $conexion->query($sql);

    if(mysqli_num_rows($resultado_egreso) > 0){
        $cont = 1;
        
        foreach ($resultado_egreso as $key):
            $html .= '<tr class="center">';
            $html .= '<td>'.($cont++).'</td>';
            $html .= '<td>'.$key['numero'].'</td>';
            $html .= '<td>'.$key['estatus'].'</td>';
            $html .= '<td>'.$key['fecha_ingreso'].'</td>';
            $html .= '<td>'.$key['fecha_egreso'].'</td>';
            $html .= '<td>'.$key['tipo_egreso'].'</td>';
            $html .= '<td>'.$key['descripcion_egreso'].'</td>';
            $html .= '</tr>';
        endforeach;
    }
...
// Descargar el PDF
$dompdf->stream('sipmg_egresos_'.$nac, array('Attachment' => true,'enable_remote' => true));
?>